...
let nextId = 0;
const SOURCE_ID = 'compaction-test';

type TestApi = 'test-compaction-api';
...
function createTestModel(reasoning: boolean, maxTokens = 8192): Model<TestApi> {
  return {
...
describe('harness compaction', () => {
  ...
  it('covers cut-point and turn-start edge cases', () => {
    ...
    expect(
      findCutPoint([thinking, branchSummary, customMessage], 0, 3, 1).firstKeptEntryIndex,
    ).toBe(0);
  ...
  it('estimates tokens and context usage across supported message roles', () => {
    ...
    expect(
      estimateTokens({ role: 'user', content: 'plain user', timestamp: Date.now() }),
    ).toBeGreaterThan(0);
    ...
    expect(
      estimateTokens({ role: 'unknown', timestamp: Date.now() } as unknown as AgentMessage),
    ).toBe(0);
    expect(
      getLastAssistantUsage([
  ...
  it('passes reasoning through generateSummary only for reasoning models with thinking enabled', async () => {
    ...
    const reasoningModel = createTestModel(true);
    registerResponses([
      (_context, options) => {
    ...
    const reasoningOffModel = createTestModel(true);
    registerResponses([
      (_context, options) => {
        seenOptions.push(options as Record<string, unknown> | undefined);
        return createSummaryMessage('## Goal\nTest summary');
    ...
    const nonReasoningModel = createTestModel(false);
    registerResponses([
      (_context, options) => {
        seenOptions.push(options as Record<string, unknown> | undefined);
        return createSummaryMessage('## Goal\nTest summary');
  ...
  it('returns error results for failed or aborted summary generations', async () => {
    ...
    registerResponses([
      () =>
        createSummaryMessage('', undefined, {
          stopReason: 'error',
          errorMessage: 'boom',
        }),
    ]);

    const errorResult = await generateSummary(messages, createTestModel(false), 2000, 'test-key');
    ...
    const abortedResult = await generateSummary(messages, createTestModel(false), 2000, 'test-key');
  ...
  it('clamps compaction summary maxTokens to the model output cap', async () => {
    ...
    registerResponses([
      ...
      (_context, options) => {
        seenOptions.push(options as Record<string, unknown> | undefined);
        return createSummaryMessage('## Original Request\nTest summary');
      },
    ]);
    ...
    getOrThrow(await compact(preparation, createTestModel(false, 128000), 'test-key'));
  ...
  it('returns compaction error results without throwing', async () => {
    ...
    expect(await compact(preparation, createTestModel(false), 'test-key')).toMatchObject({
      ok: false,
  ...
  it('returns a compaction result with file details', async () => {
    ...
    const result = getOrThrow(await compact(preparation!, createTestModel(false), 'test-key'));
  ...
  it('compacts using a hook-provided summary and emits session_compact', async () => {
    ...
    harness.subscribe((event) => {
      if (event.type === 'session_compact') events.push(`${event.type}:${event.fromHook}`);
    });
    ...
    expect(result.summary).toBe('summary from hook');
    expect(events).toEqual(['session_compact:true']);
