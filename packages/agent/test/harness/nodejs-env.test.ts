...
describe('NodeExecutionEnv', () => {
  it('reads, writes, lists, and removes files and directories', async () => {
    ...
    expect(getOrThrow(await env.joinPath([root, 'nested', 'child']))).toBe(
    ...
    expect(getOrThrow(await env.readTextLines('nested/child/file.txt', { maxLines: 1 }))).toEqual([
      'hello',
    ]);
    expect(
      Buffer.from(getOrThrow(await env.readBinaryFile('nested/child/file.txt'))).toString('utf8'),
    ).toBe('hello');
  ...
  it('returns fileInfo for files, directories, and symlinks without following symlinks', async () => {
    ...
    expect(getOrThrow(await env.canonicalPath('file-link'))).toBe(
  ...
  it('returns callback errors from exec stream handlers', async () => {
    ...
    expect(result.ok).toBe(false);
    if (!result.ok)
      expect(result.error).toMatchObject({ code: 'callback_error', message: 'callback failed' });
  });
