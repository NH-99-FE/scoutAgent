...
export interface ScoutAgentResources<
...
export interface BeforeAgentStartEvent {
  ...
  images?: ImageContent[];
  systemPrompt: string;
  resources?: ScoutAgentResources<Skill, PromptTemplate>;
}
...
export interface ScoutExtensionCommandContext extends ScoutExtensionContext {
  ...
  newSession(options?: NewSessionReplacementOptions): Promise<{ cancelled: boolean }>;
...
export interface StartUserMessageOptions {
  details?: unknown;
}
...
export interface NewSessionReplacementOptions extends SessionReplacementOptions {
...
export interface ReplacedSessionContext extends ScoutExtensionCommandContext {
  ...
  startUserMessage(
    content: string | (TextContent | ImageContent)[],
    options?: StartUserMessageOptions,
...
export interface ScoutExtensionAPI {
  ...
  setLabel(entryId: string, label: string | undefined): Promise<void>;
  getCommands(): SlashCommandInfo[];
  /** 切换模型。Scout 简化为按 modelId 选择已注册模型。 */
  setModel(modelId: string): Promise<void>;
  /** 获取当前 thinking level。 */
  getThinkingLevel(): ThinkingLevel;
  /** 切换 thinking level。 */
