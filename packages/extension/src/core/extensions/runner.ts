...
import type { ReadonlySessionManager } from '../session/index.ts';
...
export class ScoutExtensionRunner {
  ...
  createCommandContext(): ScoutExtensionCommandContext {
    ...
    context.newSession = (options) => {
      this.assertActive();
      return this.newSessionFn(options);
    };
    context.fork = (entryId, options) => {
      this.assertActive();
      return this.forkFn(entryId, options);
    };
    context.switchSession = (sessionMeta, options) => {
      this.assertActive();
      return this.switchSessionFn(sessionMeta, options);
    };
    context.reload = () => {
      this.assertActive();
