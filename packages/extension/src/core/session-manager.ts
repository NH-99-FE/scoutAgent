import { type AgentMessage, uuidv7 } from '@scout-agent/agent';
...
export type SessionTreeEntry = SessionEntry;
...
export class SessionManager {
  ...
  appendSessionName(name: string): string {
    return this.appendSessionInfo(name);
  }

  appendLabel(targetId: string, label: string | undefined): string {
    return this.appendLabelChange(targetId, label);
  }
  ...
  moveTo(
    ...
    return this.branchWithSummary(entryId, summary.summary, summary.details, summary.fromHook);
  }
   ...
   * @param sessionDir Session directory provided by the host/config layer.
   */
  ...
  static forkFrom(
    ...
    if (sourceEntries.length === 0) {
      throw new Error(
        `Cannot fork: source session file is empty or invalid: ${resolvedSourcePath}`,
      );
    }
   ...
   * @param sessionsRoot Root sessions directory provided by the host/config layer.
   * @param onProgress Optional callback for progress updates (loaded, total)
   */
  static async listAll(
    ...
    const sessionsDir = normalizePath(sessionsRoot);
