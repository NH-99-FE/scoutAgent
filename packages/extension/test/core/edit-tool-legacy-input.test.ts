...
async function createTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'scout-edit-legacy-input-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('edit tool prepareArguments', () => {
  ...
  it('prepared args execute correctly', async () => {
    ...
    const result = await definition.execute('tool-1', prepared);
    expect(result.content).toEqual([
      { type: 'text', text: 'Successfully replaced 1 block(s) in legacy.txt.' },
    ]);
