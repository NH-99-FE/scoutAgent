...
- Webview 前端按容器入口、业务 feature、基础 UI 三层组织；聊天页和设置页后续应拆成独立入口，但共享 `components/ui` 和 `lib`。
...
- `tree` 负责 session tree 交互，包括获取树、展示可见节点、navigateTree、fork、label、分支总结确认和跳回草稿节点时回填输入框。
...
- Tree：支持 session tree 展示、navigateTree、fork、label、分支总结确认；`navigate_tree_result.editorText` 存在时必须回填 composer，而不是立即发送。
...
- Tree 交互发送 `request_tree`、`navigate_tree`、`fork_session`、`set_label`，消费 `tree_data`、`navigate_tree_result`、`fork_result`、`label_result`。
