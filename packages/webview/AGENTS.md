...
- `App` 通过当前 surface 分发到：
  ...
  - `src/surfaces/changes-review/ChangesReviewApp.tsx`：Changes Review `WebviewPanel` 入口。
- surface 来源由 extension 注入 `window.__SCOUT_WEBVIEW_SURFACE__`，开发模式也支持 `?surface=chat|settings|tree|changes-review`。
- 聊天页是常驻 `WebviewView`；Settings 和 Tree 是 extension host 打开或 reveal 的 singleton `WebviewPanel`。
- Settings 是表单型 `WebviewPanel`，隐藏时默认保留 Webview 运行时；Tree 默认允许隐藏时回收运行时，重新显示后通过 `ready` 恢复 extension 权威数据。小型临时 UI 状态可用 VS Code webview `setState/getState`，禁止把完整消息、完整 tree 或工具输出等大对象塞入快照。
- 聊天页、设置页、Tree 和 Changes Review 页面由 extension host 编排，不互相耦合；它们可以共享 `components/ui`、通用 hooks/utils、store 和 shared 协议类型，但不得共享对方的页面内部组件。
...
packages/webview/src/
  ...
  surfaces/
    ...
    tree/TreeApp.tsx
    changes-review/ChangesReviewApp.tsx
  features/
    changes-review/
      model/
      view/
...
- 大型 feature 内部优先按 `view/`、`model/`、`hooks/` 分层；React 组件放 `view/`，纯投影/草稿/类型 helper 放 `model/`，带本地状态或协议副作用的 controller hook 放 `hooks/`。
...
- `changes-review` surface 负责 host 注入模型和面板消息接入；diff 展示、文件区块和 split model 属于 `features/changes-review`。
